...
public class DB01_JDBC_Update_Insert_Delete {
    public static void main(String[] args) throws SQLException {
    //JDBC更新
        //数据库操作总结起来就四个字：增删改查，行话叫CRUD：Create，Retrieve，Update和Delete。
        //
        //查就是查询，我们已经讲过了，就是使用PreparedStatement进行各种SELECT，然后处理结果集。
        // 现在我们来看看如何使用JDBC进行增删改。
    //插入
        //插入操作是INSERT，即插入一条新记录。通过JDBC进行插入，
        // 本质上也是用PreparedStatement执行一条SQL语句，不过最后执行的不是executeQuery()，
        ...
        try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
            try (PreparedStatement ps = conn.prepareStatement(
                ...
                ps.setObject(1, 1000); // 注意：索引从1开始
                ps.setObject(2, 1); // grade
                ps.setObject(3, "Bob"); // name
                ps.setObject(4, 0); // gender
                ps.setObject(5, 97); // score
                int n = ps.executeUpdate(); // 1
            }
        }
        //设置参数与查询是一样的，有几个?占位符就必须设置对应的参数。虽然Statement也可以执行插入操作，
        // 但我们仍然要严格遵循绝不能手动拼SQL字符串的原则，以避免安全漏洞。
        //
    ...
    //插入并获取主键
        ...
        //观察上述代码，有两点注意事项：
        //
        //一是调用prepareStatement()时，第二个参数必须传入常量Statement.RETURN_GENERATED_KEYS，
        // 否则JDBC驱动不会返回自增主键；
        //
        //二是执行executeUpdate()方法后，必须调用getGeneratedKeys()获取一个ResultSet对象，
        // 这个对象包含了数据库自动生成的主键的值，读取该对象的每一行来获取自增主键的值。如果一次插入多条记录，
        // 那么这个ResultSet对象就会有多行返回值。如果插入时有多列自增，那么ResultSet对象的每一行都会对应
        // 多个自增值（自增列不一定必须是主键）。
    //更新
        //更新操作是UPDATE语句，它可以一次更新若干列的记录。更新操作和插入操作在JDBC代码的层面上实际上没有区别，
        // 除了SQL语句不同：
        try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
