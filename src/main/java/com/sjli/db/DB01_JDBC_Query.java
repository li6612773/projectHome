...
public class DB01_JDBC_Query {
    public static void main(String[] args) throws SQLException {
        //JDBC查询
        //前面我们讲了Java程序要通过JDBC接口来查询数据库。
        // JDBC是一套接口规范，它在哪呢？就在Java的标准库java.sql里放着，不过这里面大部分都是接口。
        // 接口并不能直接实例化，而是必须实例化对应的实现类，然后通过接口引用这个实例。那么问题来了：JDBC接口的实现类在哪？
        //
        //因为JDBC接口并不知道我们要使用哪个数据库，所以，用哪个数据库，我们就去使用哪个数据库的“实现类”，
        // 我们把某个数据库实现了JDBC接口的jar包称为JDBC驱动。
        //
        //因为我们选择了MySQL 5.x作为数据库，所以我们首先得找一个MySQL的JDBC驱动。所谓JDBC驱动，
        // 其实就是一个第三方jar包，我们直接添加一个Maven依赖就可以了：
        //<dependency>
        ...
        //    <scope>runtime</scope>
        //</dependency>
        //注意到这里添加依赖的scope是runtime，因为编译Java程序并不需要MySQL的这个jar包，
        // 只有在运行期才需要使用。如果把runtime改成compile，虽然也能正常编译，
        // 但是在IDE里写程序的时候，会多出来一大堆类似com.mysql.jdbc.Connection这样的类，
        // 非常容易与Java标准库的JDBC接口混淆，所以坚决不要设置为compile。
        //
        //有了驱动，我们还要确保MySQL在本机正常运行，并且还需要准备一点数据。
        // 这里我们用一个脚本创建数据库和表，然后插入一些数据：
        /*
        -- 创建数据库learjdbc:
         ...
         */
        //在控制台输入mysql -u root -p，输入root口令后以root身份，把上述SQL贴到控制台执行一遍就行。
...
//      //关闭连接:
    ...
    //SQL注入
        ...
        //
        //还有一个办法就是使用PreparedStatement。使用PreparedStatement可以完全避免SQL注入的问题，
        // 因为PreparedStatement始终使用?作为占位符，
        // 并且把数据连同SQL本身传给数据库，这样可以保证每次传给数据库的SQL语句是相同的，
        // 只是占位符的数据不同，还能高效利用数据库本身对查询的缓存。上述登录SQL如果用PreparedStatement可以改写如下：
        //User login(String name, String pass) {
        //    ...
        ...
        //使用PreparedStatement和Statement稍有不同，必须首先调用setObject()设置每个占位符?的值，
        // 最后获取的仍然是ResultSet对象。
        //
        ...
        //}
    //数据类型
        //有的童鞋可能注意到了，使用JDBC的时候，我们需要在Java数据类型和SQL数据类型之间进行转换。
