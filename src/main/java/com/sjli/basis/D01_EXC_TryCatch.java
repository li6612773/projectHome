...
public class D01_EXC_TryCatch {
    public static void main(String[] args) {
    //捕获异常
        //在Java中，凡是可能抛出异常的语句，都可以用try ... catch捕获。把可能发生异常的语句放在try { ... }中，
        // 然后使用catch捕获对应的Exception及其子类。
    //多catch语句
        //可以使用多个catch语句，每个catch分别捕获对应的Exception及其子类。
        // VM在捕获到异常后，会从上到下匹配catch语句，匹配到某个catch后，执行catch代码块，然后不再继续匹配。
        //
    ...
    //finally语句
        ...
        //
        //那么如何消除这些重复的代码？Java的try ... catch机制还提供了finally语句，
        ...
        //finally总是最后执行。
        //如果没有发生异常，就正常执行try { ... }语句块，然后执行finally。如果发生了异常，
        // 就中断执行try { ... }语句块，然后跳转执行匹配的catch语句块，最后执行finally。
        //
